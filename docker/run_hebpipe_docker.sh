...
echo "docker compose -f $COMPOSE_FILE exec hebpipe [your heb_pipe command and arguments]"
...
if [ "$MOUNT_VOLUME" == "y" ]; then
    echo "docker compose -f $COMPOSE_FILE exec hebpipe $CONTAINER_MOUNT_PATH/example_in.txt"
else
    echo "docker compose -f $COMPOSE_FILE exec hebpipe example_in.txt"
fi
echo ""
